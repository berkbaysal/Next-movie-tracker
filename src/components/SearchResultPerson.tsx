...
const SearchResultPerson = ({ result }: IProps) => {
  ...
  async function getPersonImagePath(id: number) {
    ...
    if (json.profiles[0]) {
      setImgSrc('https://image.tmdb.org/t/p/w92' + json.profiles[0].file_path);
    }
  }

  useEffect(() => {
    setImgSrc(defaultPersonImage);
    getPersonImagePath(result.id);
  }, [result]);
  return (
    <div className={styles.searchResult}>
      <div className={styles.poster}>
        <Image src={imgSrc ? imgSrc : defaultPersonImage} layout="responsive" objectFit="cover" width={94} height={141} sizes="5vw" placeholder={defaultPersonImage} />
      </div>
      <div className={styles.movieInfo}>
        <div className={styles.movieTitle}>{result.name}</div>
        <div className={styles.movieOverview}>
          {result.known_for.map((work) => (
            <div className={styles.knownFor} key={result.id}>
              {work.title}
            </div>
          ))}
        </div>
